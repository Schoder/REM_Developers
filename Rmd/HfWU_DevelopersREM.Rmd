...
Liebe Studierende, sehr geeherte Mitglieder der Berufungskommission. Ich darf mich zunächst ganz herzlich für die **Einladung nach Geislingen bedanken** und freue mich sehr, in den nächsten 30min über ein sehr spannendes Thema sprechen zu dürfen.

Das Thema ist natürlich **sehr breit** angelegt und ich habe **versucht, möglichst viele auch aktuelle Themen und Zusammenhänge** aufzuzeigen. Wir werden zwar mit **einer Fallstudie auch mal etwas tiefer** in die Materie einsteigen, aber aufgrund der begrenzten Zeit werden wir vieles nur anreissen können.

Ich hoffe, aber, dass ich mit dem Vortrag einige Gedanken stimulieren kann und wir dann in der anschließend vorgesehenen Diskussion noch den einen oder anderen Aspekt vertiefen können.

Noch ein Hinweis, Sie sehen links unten meinen **github-Account**. Dort können Sie im Nachgang meine Folien anschauen, falls Sie Interesse am ein oder anderen link haben, den ich in der Präsentation vielleicht auch nur kurz anteasern kann und auf weiterführende Quellen verweisen muss.  
...
* Das Thema "Bestandsmanagement von Projektentwicklungsunternehmen" ist nicht nur sehr breit, sondern natürlich auch höchst aktuell, wie das Zitat von JLL und die aktuellen Entwicklungen in der Branche zeigen, auf die ich gleich noch zu sprechen komme.
...
.blockquote[Teil 3: Offene Fragen und Diskussion]


???

* Was habe ich in den nächsten 30min mit Ihnen vor?

* Meine Ausführungen zum Thema habe ich wie folgt strukturiert.
...
🎯 Ich kann das Geschäftsmodell von Projektentwicklungsunternehmen erläutern.

🎯 Ich bin in der Lage die Effekte von Zinsänderungen auf das Geschäftsmodell von Projektentwicklern modellhaft zu analysieren.

🎯 Ich kann begründen warum ein aktives Bestandsmanagement grundsätzlich Teil des Geschäftsmodells von Projektentwicklungsunternehmen ist.

🎯 Ich kann die besonderen Gründe für ein aktives Bestandsmanagement in der aktuellen Situation erläutern.

🎯 Ich bin in der Lage die aktuellen Herausforderungen von Projektentwicklungsunternehmen zu analysieren und strategische Implikationen abzuleiten.



???

Ganz kurz möchte ich auch noch einige Lernziele formulieren, die mit der kurzen Probevorlesung erreicht werden sollen.

Ein weiteres wichtiges Lernziel wäre vielleicht, Sie davon zu überzeugen, dass Sie insgesamt viel von mir lernen können und ich vielleicht eine ganz gute Wahl für die Besetzung der Professur wäre😄
...
.panel[.panel-name[Brutto-Anfangsrenditen]
```{r}
#| echo: false
#| fig-align: 'center'
#| out-width: '100%'
...
* Bruttoanfangsrendite: Nettojahresmiete (inkl. nicht umlagefähige Betriebskosten) im Verhältnis zum Anschaffungswert (ohne Erwerbsnebenkosten)
...
* Allein zu diesen beiden Thmen könnten ganze Vorlesungen gehalten werden. Vielleicht können wir darauf in der Diskussion noch etwas eingehen.

* An dieser Stelle möchte ich stattdessen noch kurz darauf verweisen, dass auch Wohnen und Logistikimmobilien durch die Entwicklung der jüngsten Zeit einige Hausaufgaben bekommen haben. Stichwort ESG (spielt natürlich auch bei Büro und Handel eine Rolle):
...
* Bevor wir auf diese aktuellen Themen eingehen, müssen wir uns ganz kurz noch mit einigen Begrifflichkeiten und Abgrenzungen auseinandersetzen
...
* Ich möchte deshalb nur ganz kurz auf die diversen Abgrenzungsprobleme der Begriffe eingehen und dabei gleich darauf hinweisen, dass man hier sicher 1-2 Stunden trefflich diskutieren könnte. Zu den wesentlichen Streitpunkten gehören die Abgrenzungen zwischen Portfolio- und Assetmanagement sowie zwischen Property- und Facility Management, was nicht heißt, dass ansonsten Einigkeit herrscht (auch Asset- vs. Property ist nicht immer trennscharf).
  * stellvertretend für die überwiegend auch in immobilienwirtschaftlichen Lehrbüchern vertretene Ansicht hier eine Abbildung auseiner aktuellen Veröffentlichung von Oetken/Hofstadler/Meckmann (2021). Zu beachten:
    * Sie haben versucht, die bestehende Literatur zum Thema zu synthetisieren, um dann zu Fragen, wie und wo Due Diligence in das System integriert werden kann...
...
* Beispiel UBM. Eigentlich Trading-Developer. Aufgrund der aktuellen Situation wird der Timber Pioneer in FFM aber vorerst im Bestand gehalten, bis bessere Verkaufspreise erzielt werden können.
  ...
  * UBM hat bereits Erfahrung im Betreiben von Immobilien (Tochtergesellschaft betreibt von UBM entwickelte Hotels für Markenhotellerie, bspw Holiday In Express)

* Developer haben schon länger die Attraktivität von "gebäudenahen" Dienstleistungen zur Stabilisierung von Cash-Flows für sich entdeckt (Kinateder, 2017):
  * Während Projektentwickler ihren Kunden früher eher selten Dienstleistungen für die Nutzungsphase angeboten haben (**Facility Management, Wohnungseigentums-/WEG-Verwaltung, Betreiberleistungen usw.**), 
  * haben sie heute die Vorteile der stabilen Einnahmen aus diesen Dienstleistungen entdeckt und ihre Geschäftsmodelle entsprechend erweitert. 
...
#| fig-align: 'center'
#| out-width: '80%'
knitr::include_graphics(xfun::from_root('img','challenges_dreieck.png'))
```
...
#| fig-align: 'center'
#| out-width: '60%'
knitr::include_graphics(xfun::from_root('img','degewo.png'))
```
<br>
.quellePanURL[Quelle: [degewo Berlin.](https://www.degewo.de/wohnen-service/vorteile-bei-degewo/wohnungstauschportal)]
]
.panel[.panel-name[AAL]
```{r}
#| echo: false
#| fig-align: 'center'
knitr::include_url('https://future-living-berlin.com/',height = '480px')
```
]
...
#| fig-align: 'center'
#| out-width: '100%'
#| fig-cap: "Altersstruktur von Wohngebäuden"
#| fig-topcaption: true
...
* In Anlehnung an Occupy Wallstreet. 99% der Immobilien sind Bestandsimmobilien.
  * Entwicklung
  * Altersstruktur
...
* Natürlich können die heute ausführlicher besprochenen Herausforderungen von Inlfation und höheren Renditen auch weiterhin bestehen bleiben.
* Hierüber könnten wir in einer eigenen, volkswirtschaftlichen Vorlesung, nachdenken.
* Neugierige verweise ich an dieser Stelle auf die Diskussion zwischen Lawrence Summers und Olivier Blanchard, die hier als Youtube-Video verlinkt ist.
...
* Liquidität und Flexibilität: Wer kann baut fertig und hält (gute) Objekte in der Hoffnung auf Erholung bei den Verkaufspreisen.
...
# Teil 3: Offene Fragen und Diskussion
